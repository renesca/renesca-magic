...
class HyperRelationClassSpec extends Specification with CodeComparison {
  ...
  "simple class, helper relations" >> {
    generatedContainsCode(
      ...
      q"""case class AToR(startNode: A, relation: raw.Relation, endNode: R) extends Relation[A, R]""",
      q"""case class RToB(startNode: R, relation: raw.Relation, endNode: B) extends Relation[R, B]"""
    )
  }
  ...
  "with super relation types" >> {
    generatedContainsCode(
      ...
      q"""case class R(node: raw.Node) extends HyperRelation[A, AToR, R, RToB, B] with T[A, B];"""
    )
  }
  "with super node types" >> {
    generatedContainsCode(
      q"object A {@Node trait K; @HyperRelation class R(startNode:A, endNode:B) extends K}",
      q"""case class R(node: raw.Node) extends HyperRelation[A, AToR, R, RToB, B] with K;"""
    )
  }
  "with super relation and node types" >> {
    generatedContainsCode(
      ...
      q"""case class R(node: raw.Node) extends HyperRelation[A, AToR, R, RToB, B] with T[A, B] with K;"""
    )
  }
}
